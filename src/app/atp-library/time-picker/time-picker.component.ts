...
export class TimePickerComponent implements OnInit {
    ...
    public GetTime() {
        const time = this.core.TimeToString(this.time, this.config.system);
    ...
    getDegree = (ele: any) => {
        ...
        if (this.isClicked && (ele.currentTarget === ele.target || ele.target.nodeName === 'BUTTON')) {
            ...
            const clickHit = this.core.CalcDegrees(ele, parrentPos, step);
            ...
            const nowMinHour = (+min.split(':')[0] < 12 && this.config.system === 'SYSTEM12') ? +min.split(':')[0] : +min.split(':')[0] - 12;
            const nowMaxHour = (+max.split(':')[0] < 12 && this.config.system === 'SYSTEM12') ? +max.split(':')[0] : +max.split(':')[0] - 12;
    ...
    private GetNowTime(hour: number, ampm: 'AM' | 'PM', minute: number): string {
        const Hour = (hour === 12 && ampm === 'AM' && this.config.system === 'SYSTEM12') ? '0' : hour;
        const system = (this.config.system === 'SYSTEM12' ? ' ' + ampm : '');
        const nowTime = Hour + ':' + minute + system;
        return nowTime;
    }
}
